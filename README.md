# AI 日程助手

使用 AI 解析自然语言并自动生成日历事件的智能日程助手。支持自然语言输入，AI 自动解析时间、地点、事件类型等信息，生成符合 FullCalendar 标准的日历事件。

## 项目概述

用户用自然语言描述任务（如"明天下午3点和老板开会"），AI 自动解析并生成可跨平台使用的日历事件。项目采用前后端分离架构，后端使用 OpenAI Structured Outputs 保证数据格式准确性，前端使用 Next.js + FullCalendar 提供现代化的用户界面。

### 核心特性

- ⚡ **快速解析**: "明天下午3点开会" → 2-5秒生成日程
- 🎯 **高准确度**: OpenAI Structured Outputs 保证 100% 格式正确
- 🌐 **现代化界面**: Next.js + React + FullCalendar + Tailwind CSS
- 🔧 **生产级代码**: 完整的错误处理、参数验证、日志系统
- 🌍 **代理支持**: 内置代理配置，支持中国大陆用户访问 OpenAI API
- 📱 **响应式设计**: 适配桌面和移动设备
- 🎨 **美观 UI**: 使用 shadcn/ui 组件库，支持深色模式

## 技术栈

### 后端
- **Node.js** + **Express** - Web 框架
- **OpenAI API** - 使用 Structured Outputs 进行自然语言解析
- **axios** - HTTP 客户端（支持代理）
- **FullCalendar 兼容格式** - 标准化的日历事件数据结构

### 数据流
aid
sequenceDiagram
    participant User as 用户
    participant MagicBar as MagicBar 组件
    participant API as API 客户端<br/>(schedule.ts)
    participant Express as Express 服务器
    participant CORS as CORS 中间件
    participant Logger as 日志中间件
    participant Validator as 验证中间件
    participant Controller as 解析控制器
    participant Proxy as 代理服务器<br/>(可选)
    participant OpenAI as OpenAI API
    participant Calendar as FullCalendar

    User->>MagicBar: 输入自然语言<br/>"明天下午3点开会"
    MagicBar->>MagicBar: 验证输入<br/>(长度、非空)
    MagicBar->>API: parseSchedule({text, context, options})
    API->>Express: POST /api/v1/events/parse<br/>{text, context, options}
    
    Express->>CORS: 处理跨域请求
    CORS->>Logger: 记录请求日志
    Logger->>Validator: 验证参数<br/>(text, context, options)
    
    alt 验证失败
        Validator-->>API: 400 错误响应<br/>{error: {code, message, suggestion}}
        API-->>MagicBar: ScheduleParseError
        MagicBar-->>User: 显示错误提示
    else 验证通过
        Validator->>Controller: 转发请求
        Controller->>Controller: 构建 System Prompt<br/>(基于 context 和 options)
        Controller->>Controller: 获取代理配置<br/>(如果 ENABLE_PROXY=true)
        
        loop 重试机制 (最多3次)
            Controller->>Proxy: 通过代理发送请求<br/>(如果配置了代理)
            Proxy->>OpenAI: POST /v1/chat/completions<br/>(Structured Outputs)
            OpenAI-->>Proxy: 返回结构化事件数据
            Proxy-->>Controller: 响应数据
        end
        
        alt 网络错误 (ECONNRESET/ETIMEDOUT)
            Controller->>Controller: 指数退避重试
            Controller-->>API: 网络错误响应
            API-->>MagicBar: ScheduleParseError<br/>(网络连接错误)
            MagicBar-->>User: 显示错误提示<br/>"请检查网络连接或配置代理"
        else OpenAI API 错误
            Controller-->>API: API 错误响应<br/>(400/401/429/500/503)
            API-->>MagicBar: ScheduleParseError<br/>(带错误码和建议)
            MagicBar-->>User: 显示错误提示
        else 解析成功
            Controller->>Controller: 解析 JSON 响应
            Controller->>Controller: 验证事件数据<br/>(字段完整性、时间格式、逻辑)
            Controller->>Controller: 应用限制<br/>(max_events, allow_past_events)
            Controller-->>Express: 成功响应<br/>{success: true, events, metadata}
            Express-->>API: JSON 响应
            API-->>MagicBar: ParseResponse<br/>{events, metadata}
            MagicBar->>MagicBar: 清空输入框
            MagicBar->>Calendar: 添加事件到日历<br/>(events 数组)
            Calendar-->>User: 渲染事件到日历视图
            MagicBar-->>User: 显示成功提示<br/>"成功添加 X 个日程"
        end
    end

### 前端
- **Next.js 16** - React 框架
- **React 19** - UI 库
- **FullCalendar** - 日历组件
- **Tailwind CSS** - 样式框架
- **shadcn/ui** - UI 组件库
- **Framer Motion** - 动画库
- **TypeScript** - 类型安全

## 快速开始

### 前置要求

- Node.js >= 18.0.0
- npm 或 pnpm
- OpenAI API Key（需要支持 Structured Outputs 的模型，如 `gpt-4o-2024-11-20`）
- （可选）代理服务器（中国大陆用户需要）

### 1. 克隆项目

```bash
git clone <repository-url>
cd AI-Schedule-Assistant
```

### 2. 配置后端

```bash
cd backend
npm install
```

创建 `backend/.env` 文件：

```env
# OpenAI 配置
OPENAI_API_KEY=sk-your-api-key-here
OPENAI_MODEL=gpt-4o-2024-11-20

# 服务器配置
PORT=5000
NODE_ENV=development
LOG_LEVEL=info

# 代理配置（中国大陆用户需要）
# HTTP_PROXY=http://127.0.0.1:7897
# HTTPS_PROXY=http://127.0.0.1:7897
# ENABLE_PROXY=true
```

**代理配置说明**：
- 如果在中国大陆，需要配置代理才能访问 OpenAI API
- 将 `HTTP_PROXY` 和 `HTTPS_PROXY` 设置为您的代理地址（如 Clash、V2Ray 等）
- 如果不需要代理，可以设置 `ENABLE_PROXY=false` 或注释掉代理配置

### 3. 启动后端服务

```bash
cd backend
npm start
```

后端服务将在 `http://localhost:5000` 启动。

### 4. 配置前端

```bash
cd frontend
pnpm install  # 或 npm install
```

创建 `frontend/.env.local` 文件：

```env
# 后端 API 地址
NEXT_PUBLIC_API_BASE_URL=http://localhost:5000
```

### 5. 启动前端服务

```bash
cd frontend
pnpm dev  # 或 npm run dev
```

前端服务将在 `http://localhost:3000` 启动。

### 6. 访问应用

打开浏览器访问 `http://localhost:3000`，开始使用 AI 日程助手！

## 项目结构

```
AI-Schedule-Assistant/
├── backend/                    # 后端 API
│   ├── src/
│   │   ├── config/            # 配置文件
│   │   │   └── openai.js      # OpenAI 配置
│   │   ├── controllers/       # 控制器
│   │   │   └── parse.controller.js
│   │   ├── middlewares/       # 中间件
│   │   │   ├── error-handler.js
│   │   │   ├── logger.js
│   │   │   └── validation.js
│   │   ├── prompts/           # System Prompt
│   │   │   └── system-prompt.js
│   │   ├── schemas/           # JSON Schema
│   │   │   └── event-schema.js
│   │   ├── utils/             # 工具函数
│   │   │   ├── error-codes.js
│   │   │   └── timezone-validator.js
│   │   └── app.js             # 应用入口
│   ├── tests/                 # 测试文件
│   └── package.json
├── frontend/                   # 前端应用
│   ├── app/                   # Next.js App Router
│   │   ├── page.tsx           # 主页面
│   │   ├── layout.tsx        # 布局
│   │   └── globals.css       # 全局样式
│   ├── components/            # React 组件
│   │   ├── calendar/          # 日历组件
│   │   ├── input/            # 输入组件
│   │   ├── layout/           # 布局组件
│   │   └── ui/               # UI 组件（shadcn/ui）
│   ├── lib/                  # 工具库
│   │   ├── api/              # API 调用
│   │   └── utils/            # 工具函数
│   └── package.json
├── test-page/                 # 简单测试页面（HTML + FullCalendar）
│   ├── index.html
│   └── styles.css
└── README.md
```

## API 文档

### 健康检查

```bash
GET /health
```

### 解析日程

```bash
POST /api/v1/events/parse
Content-Type: application/json

{
  "text": "明天下午3点和老板开会1小时",
  "context": {
    "current_time": "2026-01-02T10:00:00+08:00",
    "timezone": "Asia/Shanghai",
    "locale": "zh-CN"
  },
  "options": {
    "default_duration": 60,
    "allow_past_events": false,
    "max_events": 10
  }
}
```

详细 API 文档请参考 [backend/README.md](backend/README.md)

## 功能特性

### 时间解析能力

- ✅ 相对时间："明天"、"后天"、"下周"、"月底"
- ✅ 模糊时间："早上"、"下午"、"晚上"、"中午"
- ✅ 精确时间："3点"、"15:30"、"3点半"
- ✅ 时长推断：根据事件类型自动推断时长
- ✅ 全天事件：自动识别全天事件
- ✅ 重复事件：支持每日、每周、每月等重复规则

### 事件分类

- 工作（work）：会议、讨论、站会等
- 个人（personal）：约会、聚餐、娱乐等
- 健康（health）：健身、体检、运动等
- 其他（other）：其他类型事件

### 错误处理

- 完整的错误码系统（E1xxx 客户端错误，E5xxx 服务器错误）
- 友好的错误提示和建议
- 自动重试机制（网络错误时）

## 开发计划

- [x] Phase 0: 核心能力验证
- [x] Phase 1: 后端 API 实现
- [x] Phase 2: 测试页面（HTML + FullCalendar）
- [x] Phase 3: React 前端（Next.js + FullCalendar）
- [x] Phase 4: 代理支持（中国大陆用户）
- [ ] Phase 5: 数据持久化（数据库集成）
- [ ] Phase 6: 用户认证
- [ ] Phase 7: PWA 支持
- [ ] Phase 8: 日历导出（iCal、Google Calendar）

## 常见问题

### 1. 代理连接失败

如果在中国大陆，需要配置代理才能访问 OpenAI API。确保：
- 代理服务器正在运行（如 Clash、V2Ray）
- `backend/.env` 中正确配置了 `HTTP_PROXY` 和 `HTTPS_PROXY`
- 代理端口正确（默认 7897）

### 2. API Key 无效

- 确保 API Key 格式正确（以 `sk-` 开头）
- 确保 API Key 有足够的额度
- 确保使用的模型支持 Structured Outputs（如 `gpt-4o-2024-11-20`）

### 3. 前端无法连接后端

- 确保后端服务运行在 `http://localhost:5000`
- 检查 `frontend/.env.local` 中的 `NEXT_PUBLIC_API_BASE_URL` 配置
- 检查浏览器控制台的网络请求错误

## 贡献指南

欢迎提交 Issue 和 Pull Request！

## 许可证

MIT
